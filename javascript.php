...
function SnowStorm() {
  ...
  var usePNG = true;
  var imagePath = '<?php echo plugins_url('snowstorm/images/') ?>'; // relative path to snow images
  var flakeTypes = 6;
  this.flakesMax = <?php echo $snowstorm['max'] ?>;
  var flakesMaxActive = <?php echo $snowstorm['active'] ?>;
  var vMax = <?php echo $snowstorm['velocity'] ?>;
  var flakeWidth = <?php echo $snowstorm['size'] ?>;
  var flakeHeight = <?php echo $snowstorm['size'] ?>;
  var flakeBottom = <?php echo $_REQUEST['admin'] ? 48 : $snowstorm['bottom'] ?>; // Integer for fixed bottom, 0 or null for "full-screen" snow effect
  var snowCollect = <?php echo $snowstorm['collect'] ? 'true' : 'false' ?>;
  var showStatus = true;
